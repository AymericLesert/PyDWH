...
"""
This module describes the engine for SQL Server.
"""
...
class DWHConnectorDatabaseEngineSQLServer(DWHConnectorDatabaseEngine):
    """This class defines a SQL Server engine"""
    ...
    def create_table(self, table):
        ...
        request = f"""CREATE TABLE [{table.name}] ([DWHDateHeure] datetime DEFAULT CURRENT_TIMESTAMP NOT NULL,
                                                   ...
                                                   {', '.join([field.to_SQLServer() for field in table.fields.values()])}, 
                                                   {primary_key}
    ...
    def update_table(self, table):
        ...
        for field in [field for field in table.fields.values() if field.name not in existing_fields]:
            ...
            self.verbose(f"Add {field.to_SQLServer()}")
            self.execute(f"ALTER TABLE [{table.name}] ADD {field.to_SQLServer()}").close()

        for field in [field for field in table.fields.values() if field.name in existing_fields and existing_fields[field.name] != field.to_SQLServer()]:
            self.info(f"Updating field '{field.name}' ...")
            self.verbose(f"Update from {existing_fields[field.name]} to {field.to_SQLServer()}")
            self.execute(f"ALTER TABLE [{table.name}] ALTER COLUMN {field.to_SQLServer()}").close()
    ...
    def get_table(self, name):
        ...
        for column in cursor_column.fetchall():
            ...
            if column[1] not in DWHConnectorDatabaseEngineSQLServer.MAP_TYPE:
            ...
            table.add(name = column[0], 
                      type = DWHConnectorDatabaseEngineSQLServer.MAP_TYPE[column[1]], 
    ...
    def read(self, table):
        ...
        return DWHConnectorDatabaseEngineSQLServer.IteratorRecords(DWHConnectorDatabaseRecord(table), cursor, self.__dwh)
